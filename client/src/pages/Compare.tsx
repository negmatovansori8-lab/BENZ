  ...
  return (
    <div className="container-ah py-10">
      ...
      <div className="flex items-end justify-between">
        <div>
          <h1 className="font-display text-3xl">{t('compareTitle')}</h1>
          <p className="text-sm text-[var(--ah-muted)]">{t('compareSubtitle')}</p>
        </div>
        {items.length > 0 && <button type="button" className="btn-ghost" onClick={clear}>{t('clearAll')}</button>}
      </div>

      {!items.length ? (
        <div className="mt-8">
          <EmptyState icon={Scale} title={t('nothingToCompare')} text={t('addCarsToCompare')} action={{ to: '/cars', label: t('browseCars') }} />
        </div>
      ) : (
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.key} className="border-t border-[var(--ah-line)]">
                  <th className="p-3 text-left text-[var(--ah-muted)]">{t(row.label)}</th>
                  {items.map((c) => (
